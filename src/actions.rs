...
impl Tool for RecordGraph {
    ...
    fn description(&self) -> &str {
        "维护当前设计的架构图。拉出完整的模型骨架（即使用户idea只涉及改动模块，从仓库中分析而得的网络骨架因为涉及到梯度传播等与改动相关，也简明列出）参考：同时展示数据/控制/监督关系和关键接口，鼓励展示明确标为未决的模块和细节。\n\n\
...
impl Tool for RecordNote {
    ...
    fn description(&self) -> &str {
        "维护不适合只用图表达、但会影响后续协作的工作记忆：目标、硬约束、决定及理由、替代方案、风险、待查事实、验证协议和当前学习分支。使用当前支持的 set/open/parked 结构，不自创嵌套工作流 schema。\n\n\
         set 的 value 使用现有标量类型；复杂决定可用简洁字符串交代状态、选择、依据、影响和下一步。anchor 可关联已存在的图 ID 或已成功创建的 alias。写入的是信息及其确定程度，不是把助手建议变成用户确认。\n\n\
         open 和 parked 是整组替换，不是追加操作。写之前读取当前集合，保留仍有效条目；解决或撤销某项时说明原因。open 放影响当前推进的未决事项，parked 放明确暂缓而仍有价值的方向。不能为营造进展而删除未解决疑点，也不要重复堆积同一问题。\n\n\
         用户纠正或上游决定改变时更新受影响的旧笔记，保留必要的变化理由，避免新旧相反结论并存却未标明关系。不要把每句对话都写入长期状态；只有会改变后续判断、协作或交付的内容才需要记录。工具调用失败时不能宣称记录完成。"
    }
